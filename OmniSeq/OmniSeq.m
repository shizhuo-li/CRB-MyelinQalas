...
load('OmniParas.mat')
num_acq = 5;nETL = 128;
paras_epoch_1000 = double(paras_epoch_1000);
alpha = round((paras_epoch_1000(1:num_acq*nETL))*1e5)./1e5;
gap_between_readouts = round((paras_epoch_1000(num_acq*nETL+1:num_acq*nETL+num_acq))*1e5)./1e5;
TE = round((paras_epoch_1000(end-1:end))*1e5)./1e5;
...
path2use = '/autofs/space/marduk_002/users/shohei/sharing/qalas_pulseq/';
% Load ky-kz trajectory data %[kz_position ky_position index]
traj = readmatrix([path2use, 'csv_MocoQalas/SeqSmplPattern_CSGRE_240x208_R5_idx_nph5.csv']); 
traj = [traj(:,3),traj(:,2),traj(:,1)];
nETL = 128;
nTR = size(traj,1)/nETL/5;
...
time_t2_prep_after = 9.7e-3;
% TE1
T2prep_total_1 = TE(1) - time_t2_prep_after;
delay_pre_t2prep_TE1 = gap_between_readouts(1) - esp*nETL - TE(1);
delay_1_t2prep_TE1  =   round((T2prep_total_1/8 - mr.calcDuration(rf90)/2 - mr.calcDuration(t2prep_pulse)/2 + 80e-6)*1e5)./1e5;      % 11.08 ms
delay_2_t2prep_TE1  =   round(T2prep_total_1/4*1e5)./1e5;              % 25 ms
delay_3_t2prep_TE1  =   round((T2prep_total_1/8 + mr.calcDuration(rf90)/2 + mr.calcDuration(t2prep_pulse)/2 - 80e-6)*1e5)./1e5;      % 13.92 ms


% TE2
T2prep_total_2 = TE(2) - time_t2_prep_after;
delay_pre_t2prep_TE2 = gap_between_readouts(2) - esp*nETL - TE(2);
delay_1_t2prep_TE2  =   round((T2prep_total_2/8 - mr.calcDuration(rf90)/2 - mr.calcDuration(t2prep_pulse)/2 + 80e-6)*1e5)./1e5;     % 11.08 ms
delay_2_t2prep_TE2  =   round(T2prep_total_2/4*1e5)./1e5;               % 25 ms
delay_3_t2prep_TE2  =   round((T2prep_total_2/8 + mr.calcDuration(rf90)/2 + mr.calcDuration(t2prep_pulse)/2 - 80e-6)*1e5)./1e5;      % 13.92 ms
...
delay_TRinner_   =   esp - (delay_TE + mr.calcDuration(gxPre)+mr.calcDuration(gx)+mr.calcDuration(gxSpoil));
delay_TRouter   =   gap_between_readouts - esp*nETL;
delay_TRouter1   =   round((gap_between_readouts(4) - esp*nETL)*1e5)./1e5;
if num_acq>4
    delay_TRouter2   =   round((gap_between_readouts(5) - esp*nETL)*1e5)./1e5;
end
if num_acq>5
    delay_TRouter3   =   gap_between_readouts(6) - esp*nETL;
end
delT_M3_M4_      =   gap_between_readouts(3) - esp*nETL - mr.calcDuration(IRprep_pulse) - delay_IRprep;     % between end of readout#1 and start of inversion
delT_M3_M4_      =   round((delT_M3_M4_ - 0.22e-3)*1e5)./1e5;   % this 0.22 ms is needed to match IDEA
...
for iZ = 1:nDummies
    ...
    % T2 prep pulse TE 20
    seq.addBlock(mr.makeDelay(delay_pre_t2prep_TE1)); % 80e-3
    seq.addBlock(rf90,mr.makeDelay(delay_1_t2prep_TE1));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE1));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE1));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE1));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_3_t2prep_TE1));
    seq.addBlock(rf90_180PhaseOffset);
    seq.addBlock(gslSp_t2prep);

    contrast = 1;fa = alpha((contrast-1)*nETL+1:contrast*nETL);
    [rf_phase, rf_inc] = addAcq_lszR5(contrast, seq, nETL, iZ, fa, adc, rfSpoilingInc, rf_phase, rf_inc, stepsZ, stepsY, gxPre, gx, gxSpoil, delay_TRinner_, gyPre,gyReph,gzPre,gzReph,1,sys);

    % T2 prep pulse TE 80
    seq.addBlock(mr.makeDelay(delay_pre_t2prep_TE2)); % 20e-3
    seq.addBlock(rf90,mr.makeDelay(delay_1_t2prep_TE2));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE2));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE2));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE2));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_3_t2prep_TE2));
    seq.addBlock(rf90_180PhaseOffset);
    seq.addBlock(gslSp_t2prep);

    % Contrast 2
    contrast = 2;fa = alpha((contrast-1)*nETL+1:contrast*nETL);
    [rf_phase, rf_inc] = addAcq_lszR5(contrast, seq, nETL, iZ, fa, adc, rfSpoilingInc, rf_phase, rf_inc, stepsZ, stepsY, gxPre, gx, gxSpoil, delay_TRinner_, gyPre,gyReph,gzPre,gzReph,1,sys);
    seq.addBlock(mr.makeDelay(delT_M3_M4_));
    ...
    % Contrast 3
    contrast = 3;fa = alpha((contrast-1)*nETL+1:contrast*nETL);
    [rf_phase, rf_inc] = addAcq_lszR5(contrast, seq, nETL, iZ, fa, adc, rfSpoilingInc, rf_phase, rf_inc, stepsZ, stepsY, gxPre, gx, gxSpoil, delay_TRinner_, gyPre,gyReph,gzPre,gzReph,1,sys);

    seq.addBlock(mr.makeDelay(delay_TRouter1))

    % Contrast 4
   contrast = 4;fa = alpha((contrast-1)*nETL+1:contrast*nETL);
    [rf_phase, rf_inc] = addAcq_lszR5(contrast, seq, nETL, iZ, fa, adc, rfSpoilingInc, rf_phase, rf_inc, stepsZ, stepsY, gxPre, gx, gxSpoil, delay_TRinner_, gyPre,gyReph,gzPre,gzReph,1,sys);

    seq.addBlock(mr.makeDelay(delay_TRouter2));

    % Contrast 5
    contrast = 5;fa = alpha((contrast-1)*nETL+1:contrast*nETL);
    [rf_phase, rf_inc] = addAcq_lszR5(contrast, seq, nETL, iZ, fa, adc, rfSpoilingInc, rf_phase, rf_inc, stepsZ, stepsY, gxPre, gx, gxSpoil, delay_TRinner_, gyPre,gyReph,gzPre,gzReph,1,sys);
...
for iZ = 1:nTR
    ...
    % T2 prep pulse TE 20
    seq.addBlock(mr.makeDelay(delay_pre_t2prep_TE1));
    seq.addBlock(rf90,mr.makeDelay(delay_1_t2prep_TE1));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE1));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE1));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE1));
    ...
    % T2 prep pulse TE 80
    seq.addBlock(mr.makeDelay(delay_pre_t2prep_TE2));
    seq.addBlock(rf90,mr.makeDelay(delay_1_t2prep_TE2));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE2));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE2));
    seq.addBlock(t2prep_pulse,mr.makeDelay(delay_2_t2prep_TE2));
...
addpath /autofs/cluster/berkin/berkin/Matlab_Code_New/pulseq/ISMRM-Virtual-Meeting--November-15-17-2023-main/tutorials/day2_SMS-EPI/safe_pns_prediction-master
...
time_perTR = sum(gap_between_readouts)+delT_M13_2end;
scan_duration = (nDummies + nTR) * time_perTR;
